...
**⚠️ Limite rígido de 13 circuitos no legado.** O `template.docx` não itera essa tabela — são linhas fixas `circuito0`…`circuito12` no Word. O código injeta quantas chaves houver via loop, mas **do 14º circuito em diante os dados são descartados silenciosamente**, sem erro nem aviso. Instalações com quadros grandes perdem circuitos no documento final sem que ninguém perceba. Na nova stack, a tabela de circuitos itera de verdade (sem teto) em `src/document/sections.rs` — ver [`report-template.md`](report-template.md) para a evidência primária no template legado.
...
**`report_section` (funcionalidade nova, sem equivalente no legado).** Campo opcional que associa a foto à seção do laudo que ela ilustra (`inspection_planning`, `external_influences`, `qualitative_assessment`, `quantitative_assessment` ou `circuits`); `NULL` é imagem geral, cai no apêndice final. Eixo independente de `finding_category` — uma foto pode ter os dois, só um, ou nenhum. Resolve uma limitação real do legado: lá toda foto caía junto no apêndice, mesmo quando ilustrava um achado de uma seção específica. Mesmo padrão de `finding_category`: `TEXT` livre validado na aplicação (`domain::REPORT_SECTIONS`), não enum de banco.
...
- **Elementos de responsabilidade técnica no documento**: **decidido incluir** capa, cabeçalho institucional, campo de ART e assinatura/parecer conclusivo — nenhum existe no legado (era só o miolo de tabelas, recortado de um documento maior). É **funcionalidade nova**, desenhada do zero no `itui` — são elementos de diagramação, não de redação (ver fronteira abaixo).
- **Redação do texto do laudo é responsabilidade do `raijin`, não do `itui` (revisão desta decisão).** Registro anterior desta seção dizia "geração client-side do documento" para o limite de circuitos e para os elementos de responsabilidade técnica — isso permanece válido só para **diagramação e exportação** (PDF/DOCX, capa, ART, assinatura, conversão para o editor TipTap). A partir do Step 5 (`src/document/`, `src/llm/`), **redigir o texto** — tanto o modelo padrão determinístico quanto o assistido por IA — é o backend: os dois caminhos compartilham o mesmo esqueleto de seções e os mesmos rótulos pt-BR, saem em Markdown, e o `itui` só injeta esse Markdown no `document_content` (TipTap). Ver `docs/report-template.md` e `docs/api-contract.md` (`GET .../draft`, `POST .../generate`).

...
- **`.env` nunca committado.** Segredos (`JWT_SECRET`, `TASK_TOKEN`, `DATABASE_URL`) só via variável de ambiente — o legado tinha credenciais em texto puro no repo, não repetir. `GOOGLE_CLIENT_ID` não é segredo (vai no bundle do `itui` por design, é o client secret que não existe neste fluxo — sem authorization-code flow, sem crate `oauth2`).
...
**Migrations são append-only.** O primeiro `sqlx migrate run` contra o Neon de produção já aconteceu, então a antiga liberação de squash caducou: **nunca edite uma migration existente**, nem a `0001_initial.sql`. Toda mudança de schema é um arquivo novo (`0002_...`, `0003_...`), escrito pra aplicar com segurança sobre linhas que já existem — coluna nova entra nullable ou com `DEFAULT`, nunca `NOT NULL` sem default. `docker compose down -v` deixou de ser a rotina: rode `sqlx migrate run` **sobre** o banco local existente, que é exatamente o que vai acontecer no Neon. Migration roda contra a **conexão direta**, não o pooler.

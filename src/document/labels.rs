...
pub const INSPECTION_PLANNING: &[(&str, &str)] = &[
    (
        ...
        "Qual a qualificação profissional dos responsáveis pela inspeção?",
    ),
    (
        "team_fit_for_work",
    ...
    (
        "has_nr10_training",
        "Um ou mais executores da inspeção possui curso NR-10?",
    ),
    (
        "service_pre_checked",
        "O serviço foi preliminarmente conferido?",
    ),
    ("identified_hazards", "Quais riscos foram detectados?"),
    (
        "safety_equipment",
        "Quais equipamentos de segurança serão utilizados?",
    ),
    (
        "requires_shutdown",
    ...
    (
        ...
        "Necessita delimitar a área de trabalho?",
    ),
    (
        "requires_utility_assistance",
        "Necessita de auxílio de concessionária local?",
    ),
    (
        "requires_voltage_check",
        "Necessário fazer verificação de tensão?",
    ),
    (
        "requires_temporary_grounding",
        "A inspeção requer aterramento temporário?",
    ),
    ("work_at_height", "A inspeção será realizada em altura?"),
    (
        "requires_safety_harness",
...
pub const EXTERNAL_INFLUENCES: &[(&str, &str)] = &[
    ...
    (
        ...
        "Resistência elétrica do corpo humano no ambiente",
    ),
    (
    ...
    (
        ...
        "Qual a natureza dos materiais de construção",
    ),
    (
        "building_structure_class",
        "Qual a classificação da estrutura das edificações",
    ),
];
...
pub const QUALITATIVE_ASSESSMENT: &[(&str, &str)] = &[
    ...
    (
        "spare_circuit_capacity",
        "Há disponibilidade de criação de novos circuitos no quadro de distribuição?",
    ),
    (
    ...
    (
        ...
        "Qual o esquema de aterramento utilizado?",
    ),
    (
        "has_backup_power_source",
        "Existe fonte alternativa ou de reserva de energia?",
    ),
    (
        "has_safety_power_source",
        "Existe fonte de segurança de energia?",
    ),
    (
...
pub fn nbr_class_label(field: &str, code: &str) -> String {
    crate::domain::label_of(field, code)
        .map(str::to_string)
